...
bool RFM69_sendWithRetry(uint8_t toAddress, const void *buffer, uint8_t bufferSize, uint8_t retries,
                         uint8_t retryWaitTime); // 40ms roundtrip req for 61byte packets
...
void RFM69_setMode(uint8_t newMode) {
  ...
  while (_mode == RF69_MODE_SLEEP &&
         (RFM69_readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00); /// wait for ModeReady
...
void RFM69_setHighPower(bool onOff) {
  ...
  if (ISRFM69HW) /** turning ON */
    RFM69_writeReg(REG_PALEVEL, (RFM69_readReg(REG_PALEVEL) & 0x1F) | RF_PALEVEL_PA1_ON |
  ...
  const uint8_t CONFIG[][2] =
    {
      ...
      /** 0x01 */
                  /// { REG_OPMODE, RF_OPMODE_SEQUENCER_ON | RF_OPMODE_LISTEN_ON | RF_OPMODE_STANDBY },
      /** 0x01 */
      ...
      /** 0x07 */
                  {REG_FRFMSB,        (uint8_t) (freqBand == RF69_315MHZ ? RF_FRFMSB_315 : (freqBand == RF69_433MHZ
                                                                                           ? RF_FRFMSB_433 : (
                                                                                             freqBand == RF69_868MHZ
                                                                                             ? RF_FRFMSB_868
                                                                                             : RF_FRFMSB_915)))},
      /** 0x08 */
                  {REG_FRFMID,        (uint8_t) (freqBand == RF69_315MHZ ? RF_FRFMID_315 : (freqBand == RF69_433MHZ
                                                                                           ? RF_FRFMID_433 : (
                                                                                             freqBand == RF69_868MHZ
                                                                                             ? RF_FRFMID_868
                                                                                             : RF_FRFMID_915)))},
      /** 0x09 */
                  {REG_FRFLSB,        (uint8_t) (freqBand == RF69_315MHZ ? RF_FRFLSB_315 : (freqBand == RF69_433MHZ
                                                                                           ? RF_FRFLSB_433 : (
                                                                                             freqBand == RF69_868MHZ
                                                                                             ? RF_FRFLSB_868
                                                                                             : RF_FRFLSB_915)))},
      ...
      /** 0x2F */
      /// { REG_SYNCVALUE1, (uint8_t)(networkID & 0x00FF) },  // NETWORK ID lower 8 bits

      /** 0x30 */
                 {REG_SYNCVALUE2,    networkID}, /// NETWORK ID
      /** 0x30 */
      /// { REG_SYNCVALUE2, (uint8_t)(networkID >> 8) },      // NETWORK ID higher 8 bits
      /** 0x37 */
...
*/
  ...
  while (((RFM69_readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00) &&
         !Timeout_IsTimeout1()); // wait for ModeReady
...
uint32_t RFM69_getFrequency() {
  return RF69_FSTEP * (((uint32_t) RFM69_readReg(REG_FRFMSB) << 16) + ((uint16_t) RFM69_readReg(REG_FRFMID) << 8) +
...
bool RFM69_canSend() {
  if (_mode == RF69_MODE_RX && payloadLen == 0 &&
      RFM69_readRSSI(0) < CSMA_LIMIT) // if signal stronger than -100dBm is detected assume channel activity
  {
    RFM69_setMode(RF69_MODE_STANDBY);
    return true;
  }
...
void RFM69_send(uint8_t toAddress, const void *buffer, uint8_t bufferSize, bool requestACK) {
  RFM69_writeReg(REG_PACKETCONFIG2,
                 (RFM69_readReg(REG_PACKETCONFIG2) & 0xFB) | RF_PACKET2_RXRESTART); // avoid RX deadlocks
  //uint32_t now = millis();
...
bool
RFM69_sendWithRetry(uint8_t toAddress, const void *buffer, uint8_t bufferSize, uint8_t retries, uint8_t retryWaitTime) {
...
static void RFM69_sendFrame(uint8_t toAddress, const void *buffer, uint8_t bufferSize, bool requestACK, bool sendACK) {
  ...
  while (RFM69_ReadDIO0Pin() == 0 &&
         !Timeout_IsTimeout1()); /// wait for DIO0 to turn HIGH signalling transmission finish
...
void RFM69_interruptHandler() {
  ...
  if ((_mode == RF69_MODE_RX || _mode == RF69_MODE_LISTEN) &&
      (RFM69_readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY)) {
    uint8_t CTLbyte;
    ...
    if (!(_promiscuousMode || targetID == _address || targetID ==
...
void RFM69_listen(bool onOff) {
  if (onOff == true) {
    ...
    //writeReg(REG_OPMODE, RF_OPMODE_LISTEN_ON | RF_OPMODE_STANDBY);
